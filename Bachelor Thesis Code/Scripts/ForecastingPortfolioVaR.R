...
uni_normal_spec <- ugarchspec(variance.model = uni_normal_varModel,
                              mean.model = uni_normal_meanModel,
                              distribution.model = "norm")
cl <- makePSOCKcluster(numcores)
uni_normal_GARCH_roll <- ugarchroll(uni_normal_spec, portfolio_plret_ts, 
                                    refit.every = 1, refit.window = "moving", 
                                    n.start = 1000, solver = "hybrid",
...
uni_ewma_spec <- ugarchspec(variance.model= ewma_varModel, 
                            mean.model= ewma_meanModel,  
                        distribution.model="norm", 
                        fixed.pars=list(omega=0, alpha = 1-lambda))
cl <- makePSOCKcluster(numcores)
uni_ewma_roll <- ugarchroll(uni_ewma_spec, portfolio_plret_ts, n.start = 1000,
                            refit.every = 1,
...
uni_t_gjr_spec <- ugarchspec(variance.model = uni_t_gjr_varModel,
                             mean.model = uni_t_gjr_meanModel, 
                             distribution.model = "std")
cl <- makePSOCKcluster(numcores)
uni_t_gjr_GARCH_roll <- ugarchroll(uni_t_gjr_spec, portfolio_plret_ts, 
                              n.start = 1000, refit.every = 1,
                              refit.window = "moving", solver = "hybrid",
                              calculate_var = TRUE, VaR.alpha = c(0.01, 0.05),
                              cluster = cl, keep.coef = FALSE)
stopCluster(cl)
...
uni_skewt_gjr_spec <- ugarchspec(variance.model = uni_skewt_gjr_varModel,
                                 mean.model = uni_skewt_gjr_meanModel, 
                                 distribution.model = "sstd")
cl <- makePSOCKcluster(numcores)
uni_skewt_gjr_GARCH_roll <- ugarchroll(uni_skewt_gjr_spec, portfolio_plret_ts, 
                                n.start = 1000, refit.every = 1,
                                refit.window = "moving", solver = "hybrid",
                                calculate_var = TRUE, VaR.alpha = c(0.01, 0.05),
                                cluster = cl, keep.coef = FALSE)
...
ms2garch_VaR_df <- data.frame(Date = portfolio_plret_df[-c(1:1000),1],
                           alpha_1perc = ms2garch_VaR[,1], 
                           alpha_5perc = ms2garch_VaR[,2])
...
dcc_uspec <- ugarchspec(variance.model = dcc_varModel, 
                        mean.model = dcc_meanModel, distribution.model = "norm")
dcc_mspec <- multispec(replicate(10, dcc_uspec))
dcc_spec <- dccspec(dcc_mspec, VAR = FALSE, model = "DCC", dccOrder = c(1,1), 
                    distribution =  "mvnorm")

cl <- makePSOCKcluster(numcores)
multi_dcc_roll <- dccroll(spec = dcc_spec, data = stocks_plret_ts, n.ahead = 1, 
                          n.start = 1000, refit.every = 1, 
