...
start.time <- Sys.time()
cl <- makeCluster(8)
roll.dcc <- dccroll(spec = dccspec, data = stocks.plret.ts, n.ahead = 1, n.start = 1000, refit.every = 1, refit.window = "moving", window.size = 1000,  cluster = cl)
stopCluster(cl)
end.time <- Sys.time()
end.time-start.time
n <- length(stocks.perclret.df[,1])
...
dcc.sigma <- rcov(roll.dcc) # rcov to extract forecasted Covariance matrix; rcor would extract forecasted Correlation matrix
for (i in 1:n.window) sigma.pf[i]<-portfolio.weights %*% dcc.sigma[,,i] %*% portfolio.weights
VaR.dcc <- sapply(sigma.pf, function(x)x*qnorm(0.01, 0, 1, lower.tail = TRUE))
```
