...
m <- 100
n.window <- length(stocks.perclret.df[,1])-1000
...
VaR[1:835]
plot(VaR)
FFCportfolio.ts <- xts(apply(FFCFactors.ts, 1, mean), order.by = (FFCFactors.df[,1]))

VaR.ts <- xts(VaR[1:835], order.by = FFCFactors.df[1:835,1])
as.numeric(VaR[1:835])
end <- length(stocks.plret.df[,1])
start <- end-835
VaRplot(0.05, FFCportfolio.ts[start:end], VaR.ts)


#### forgot reverse trafo of simulated values
VaRTest(alpha = 0.05, FFCportfolio.ts[1:835], VaR.ts, 0.95)
VaR.df <- data.frame(VaR.ts)
write.csv(VaR.df, "../Old Test Code\\VaRCopula-GARCH.csv", row.names = TRUE)
```
