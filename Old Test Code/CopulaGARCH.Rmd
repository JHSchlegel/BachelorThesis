...
for (i in 1:10){
  ...
  assign(name, xts(stocks.plret.df[,i+1], order.by = stocks.plret.df$Date))
}

stocks.ts <- xts(stocks.plret.df[,-1], order.by = stocks.plret.df$Date)
stocks.ts
...
n.ahead <- 1
n <- length(stocks.plret.df[,-1])-n.ahead+1
meanModel <- list(armaOrder = c(3, 0))
varModel <- list(model = "fGARCH", submodel = "NGARCH", garchOrder = c(1,1))
uspec <- ugarchspec(varModel, mean.model = meanModel, distribution.model = "sstd")
...
```{r}
FFCFactors.ts <- xts(FFCFactors.df[,-c(1,5)], order.by = as.Date(FFCFactors.df$Date))
set.seed(42)
numcores <- detectCores()
...
cop.sim.df <- data.frame(cop.sim)
p <- ggplot(cop.sim.df, aes(cop.sim.df[,1],cop.sim.df[,2]))+
  geom_point()+
  xlab("U")+ylab("V")+ggtitle(substitute(paste(copula_with , rho==cor)))+
  theme_classic()+
  stat_ellipse(level = 0.5, color = "red")+
  stat_ellipse(level = 0.7, color = "red")+
...
res.sim.df <- as.data.frame(res.sim)
p <- ggplot(res.sim.df, aes(res.sim.df[,1], res.sim.df[,2]))+
  geom_point()+
  xlab("U")+ylab("V")+ggtitle(substitute(paste(copula_with , rho==cor)))+
  theme_classic()+
  stat_ellipse(level = 0.5, color = "red")+
